...
def send_company_email(request: Request):
    ...
    response = requests.post(
            ...
            data={"from": f"Mailgun Sandbox <{os.getenv('MAILGUN_SENDER_EMAIL')}>",
                "to": f"<{os.getenv('MAINGUN_RECEIVER_EMAIL')}>",
                "subject": str(request.data.get('subject')),
                "text": str(request.data.get('message'))})
    if response.status_code == 200:
        return Response({"status":"success", "info":"email send successfully"}, status=200)
    else:
        return Response({"status":"failure", "info":"email not send successfully"}, status=400)