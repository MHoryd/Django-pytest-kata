...
def test_proper_response_on_success(mock_post):
...
@patch('api.sample_app.companies.views.requests.post')
def test_proper_response_on_failure(mock_post):
    ...
    assert response.json() == {"status":"failure", "info":"email not send successfully"}


def test_get_method_should_fail():
